...
# sample client which manipulates router1 device
...
def main():
    ...
    # get one device
    print('getting router1')
    result = getDevice(BASE_URL + 'get?hostname=router1')
    if result[0] != status.HTTP_200_OK:
        handleFail(result)

    print(result[1]) # print the device

    # set router 1's ip address
    print('setting router1\'s ip address')
    newIPAddr = '127.10.10.1'
    result = setIPAddress(token, 'router1', newIPAddr)
    if result[0] != status.HTTP_200_OK:
        handleFail(result)
        
    # print(result[1]) # debug

    # get router1's configuration to verify
    print('getting router1 again to verify')
    result = getDevice(BASE_URL + 'get?hostname=router1')
    if result[0] != status.HTTP_200_OK:
        handleFail(result)
